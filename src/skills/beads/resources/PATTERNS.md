...
4. **Resume execution**:
   ```
   "From bd notes: JWT validation is done with 7-day refresh tokens. Currently implementing rotation endpoint. Next: add rate limiting and tests."
   ```
   Continue with that next step immediately unless the notes reveal a real blocker, contradiction, or missing decision that truly requires user input.
...
2. Create TodoWrite:
   ...
   ```
3. Continue with the next concrete step from notes
4. Only stop to ask the user if the notes expose a real blocker, conflicting acceptance, or a missing decision that tools and repo context cannot resolve
