...
- [ ] Reconstruct the next concrete TodoWrite tasks from notes
- [ ] Continue execution immediately when NEXT is actionable
- [ ] Ask the user only if notes reveal a real blocker, contradictory acceptance, or a missing decision that cannot be resolved from tools and repo context
```

**Pattern**: Notes field is the "read me first" guide for resuming work.

### At Session End (Claude records resumable state)

When wrapping up work on an issue:

```
Session End Handoff:
- [ ] Notice work reaching a logical stopping point
- [ ] Confirm the next governed step is NOT still actionable right now from tools and repo context
- [ ] Write current-state notes with COMPLETED/IN_PROGRESS/NEXT
- [ ] Include blocker if truly blocked
- [ ] Include key decisions if relevant
- [ ] Execute the bd update directly
- [ ] Confirm notes are saved for next-session resumption
```
...
- Notes document a legitimate stop; they do **not** authorize stopping while `NEXT` is still actionable in the current session
- Ending on tool output, thinking-only output, or placeholder progress text is not a logical stopping point
...
- [ ] Confirm `NEXT` cannot be executed truthfully in this same session
...
```
User Tips:
- [ ] When stopping work: Let Claude save resumable notes
- [ ] When resuming: Let Claude read notes and continue from the recorded NEXT step
...
# Claude reconstructs the TodoWrite list from NEXT and resumes batchUpdate implementation immediately.
```
