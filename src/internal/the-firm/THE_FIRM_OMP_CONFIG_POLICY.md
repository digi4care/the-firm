# The Firm OMP Config Policy v0

## Purpose
This document defines how The Firm relates to `omp` runtime configuration.

The Firm does not treat `config.yaml` as mere tooling trivia. OMP runtime settings materially affect:
- model behavior
- context compaction
- delegation patterns
...
OMP is the runtime substrate. The Firm is the governing layer.

That means:
- OMP config is not allowed to silently undermine The Firm doctrine
...
## Policy categories
The Firm classifies OMP settings into four categories:
...
## Model role policy
### Why it matters
The Firm already differentiates roles such as planning, architecture, delivery, QA, and client-facing coordination. OMP model-role routing should reinforce that rather than flatten it.
...
- validate current OMP config against The Firm policy
...
## Summary
The Firm must read OMP config as part of runtime governance, not as an implementation afterthought.
