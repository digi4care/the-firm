# The Firm OMP Override Model v0

## Purpose
This document defines how The Firm overrides OMP’s default workflow behavior.

The Firm does not treat OMP as a complete operating model. It treats OMP as the runtime in which a stricter, issue-driven, role-specialized system operates.

This document therefore defines how The Firm replaces OMP’s default assumptions across:
- context
- agents
...
OMP remains the execution substrate.
The Firm becomes the governing layer.

That means:
- OMP tools still execute work
- OMP sessions still host agent reasoning
- OMP subagents still perform specialized tasks
...
The Firm overrides OMP at six layers.

### 1. Context override
Default OMP behavior assumes a general coding-assistant context model.
...
### 2. Agent override
Default OMP behavior can operate with broad, generalist agent patterns.
...
### 3. Workflow override
Default OMP behavior allows useful work to happen with relatively flexible sequencing.
...
Default OMP behavior may stop at implementation plus ad hoc checking.
...
Default OMP behavior can operate primarily out of session context, file context, and ad hoc task decomposition.
...
Default OMP behavior permits direct task delegation whenever useful.
...
## Runtime role of OMP
Within The Firm, OMP is responsible for:
...
OMP is not the source of governance truth.
Governance truth lives in The Firm doctrine plus the issue tracker and versioned workflow state.

## Override mechanisms
The Firm overrides OMP through a combination of mechanisms.
...
The Firm defines its own specialist agents that OMP discovers and runs.
...
## Behavioral differences from default OMP
### Default OMP tendency
...
OMP agents in The Firm are grouped into offices rather than a flat utility list.
...
Any OMP runtime operating under The Firm must preserve the following truths:
...
In greenfield repos, The Firm should override OMP from the start.
...
To implement The Firm inside OMP, the project will eventually need:
...
## Summary
The Firm does not replace OMP as software.
It replaces OMP’s default operating assumptions.

OMP remains the runtime engine.
