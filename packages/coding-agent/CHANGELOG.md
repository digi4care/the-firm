...
- Fixed monorepo `tsconfig.json` path mappings to resolve `@digi4care/the-firm-ai` subpath exports to source files in development checkouts ([#2625](https://github.com/badlogic/pi-mono/pull/2625) by [@ferologics](https://github.com/ferologics))
...
- Added missing `ajv` direct dependency; previously relied on transitive install via `@digi4care/the-firm-ai` which broke standalone installs ([#2252](https://github.com/badlogic/pi-mono/issues/2252))
...
- Built-in tools now work like custom tools in extensions. To get built-in tool definitions, import `readToolDefinition` / `createReadToolDefinition()` and the equivalent `bash`, `edit`, `write`, `grep`, `find`, and `ls` exports from `@digi4care/the-firm`.
...
- Added `ToolCallEventResult` to the `@digi4care/the-firm` top-level and core extension exports so extension authors can type explicit `tool_call` handler return values ([#2458](https://github.com/badlogic/pi-mono/issues/2458))
...
- Faster startup by lazy-loading `@digi4care/the-firm-ai` provider SDKs on first use instead of import time ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Lazy-load built-in `@digi4care/the-firm-ai` provider modules and root provider wrappers so coding-agent startup no longer eagerly loads provider SDKs before first use ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Fixed `@digi4care/the-firm-ai/oauth` extension imports in published installs by resolving the subpath directly from built `dist` files instead of package-root wrapper shims ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level `@digi4care/the-firm-ai` entry. OAuth login and refresh must be imported from `@digi4care/the-firm-ai/oauth` ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed extension loading with the new `@digi4care/the-firm-ai/oauth` export path by aliasing the oauth subpath in the extension loader and development path mapping ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed browser bundling compatibility for `@digi4care/the-firm-ai` by removing Node-only side effects from default browser import paths ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
## [0.55.4] - 2026-03-02
...
- **Android/Termux support**: Pi now runs on Android via Termux. Install with:
  ...
  npm install -g @digi4care/the-firm
  mkdir -p ~/.pi/agent
...
- Extensions now load correctly in compiled Bun binary using `@mariozechner/jiti` fork with `virtualModules` support. Bundled packages (`@sinclair/typebox`, `@digi4care/the-firm-tui`, `@digi4care/the-firm-ai`, `@digi4care/the-firm`) are accessible to extensions without filesystem node_modules.
...
```typescript
// Before (hook)
import type { HookAPI, HookContext } from "@digi4care/the-firm";
export default function (pi: HookAPI) { ... }

// Before (custom tool)
import type { CustomToolFactory } from "@digi4care/the-firm";
...
- **Key detection functions removed from `@digi4care/the-firm-tui`**: All `isXxx()` key detection functions (`isEnter()`, `isEscape()`, `isCtrlC()`, etc.) have been removed. Use `matchesKey(data, keyId)` instead (e.g., `matchesKey(data, "enter")`, `matchesKey(data, "ctrl+c")`). This affects hooks and custom tools that use `ctx.ui.custom()` with keyboard input handling. ([#405](https://github.com/badlogic/pi-mono/pull/405))
...
- `Attachment` type removed. Use `ImageContent` from `@digi4care/the-firm-ai` instead. Add images directly to message content arrays.
...
- **SDK API changes** ([#296](https://github.com/badlogic/pi-mono/issues/296)):
  ...
  - Use `getModel()` from `@digi4care/the-firm-ai` for built-in models, `modelRegistry.find()` for custom models + built-in models
...
- **OAuth and model config exports**: Scripts using `AgentSession` directly can now import `getAvailableModels`, `getApiKeyForModel`, `findModel`, `login`, `logout`, and `getOAuthProviders` from `@digi4care/the-firm` to reuse OAuth token storage and model resolution. ([#245](https://github.com/badlogic/pi-mono/issues/245))
...
- **`getMarkdownTheme()` export**: Custom tools can now import `getMarkdownTheme()` from `@digi4care/the-firm` to use the same markdown styling as the main UI.
...
- Hooks loader now supports same import aliases as custom tools (`@sinclair/typebox`, `@digi4care/the-firm-ai`, `@digi4care/the-firm-tui`, `@digi4care/the-firm`).
...
- Fixed custom tools failing to load from `~/.pi/agent/tools/` when pi is installed globally. Module imports (`@sinclair/typebox`, `@digi4care/the-firm-tui`, `@digi4care/the-firm-ai`) are now resolved via aliases.
...
- Updated `@digi4care/the-firm-ai` with X-Initiator header support for GitHub Copilot, ensuring agent calls are not deducted from quota. ([#200](https://github.com/badlogic/pi-mono/pull/200) by [@kim0](https://github.com/kim0))
...
- Updated `@digi4care/the-firm-ai` with interleaved thinking enabled by default for Anthropic Claude 4 models.
...
- Updated `@digi4care/the-firm-ai` with interleaved thinking support for Anthropic models.
