...
<context-loading>
If `.firm/navigation.md` exists, load project standards to check compliance:

1. Read `.firm/navigation.md` to discover available knowledge areas.
2. When reviewing `.firm/` content: load `.firm/lookup/standards/navigation.md` for frontmatter, naming, and format compliance.
3. When reviewing code that relates to architecture: load `.firm/concepts/decisions/navigation.md` for ADR compliance.

Maximum 2 context files. Skip if `.firm/` does not exist.

When reviewing code, also load relevant standards from `.firm/lookup/standards/` if they apply to the files being reviewed.
</context-loading>
