...
export interface Settings {
	...
	read?: {
		defaultLimit?: number;
		prosechunks?: boolean;
		explorechunks?: boolean;
		anchorstyle?: "full" | "kind" | "checksum";
	};
	bashInterceptor?: {
		enabled?: boolean;
	};
	python?: {
		toolMode?: "both" | "notebook" | "script";
		kernelMode?: "session" | "ephemeral";
		sharedGateway?: boolean;
	};

	// ─── Interaction/P4 settings (the-firm-anj) ────────────
	...
	stt?: {
		enabled?: boolean;
		language?: string;
		modelName?: "tiny" | "tiny.en" | "base" | "base.en" | "small" | "small.en" | "medium" | "medium.en" | "large";
	};

	// ─── Sampling settings (the-firm-hvf) ──────────────────
	sampling?: {
		temperature?: number;
		topP?: number;
		topK?: number;
...
export class SettingsManager {
	...
	// ─── Editing: fuzzy match ────────────────────────────────
	getEditFuzzyMatch = (): boolean => (this.get("edit.fuzzyMatch") ?? true) as boolean;
	setEditFuzzyMatch = (enabled: boolean) => this.set("edit.fuzzyMatch", enabled);

	getEditFuzzyThreshold = (): number => (this.get("edit.fuzzyThreshold") ?? 0.95) as number;
	setEditFuzzyThreshold = (value: number) => this.set("edit.fuzzyThreshold", value);

	getEditStreamingAbort = (): boolean => (this.get("edit.streamingAbort") ?? false) as boolean;
	setEditStreamingAbort = (enabled: boolean) => this.set("edit.streamingAbort", enabled);

	getEditBlockAutoGenerated = (): boolean => (this.get("edit.blockAutoGenerated") ?? true) as boolean;
	setEditBlockAutoGenerated = (enabled: boolean) => this.set("edit.blockAutoGenerated", enabled);

	// ─── Editing: read options ───────────────────────────────
	getReadDefaultLimit = (): number => (this.get("read.defaultLimit") ?? 300) as number;
	setReadDefaultLimit = (value: number) => this.set("read.defaultLimit", value);

	getReadProsechunks = (): boolean => (this.get("read.prosechunks") ?? false) as boolean;
	setReadProsechunks = (enabled: boolean) => this.set("read.prosechunks", enabled);

	getReadExplorechunks = (): boolean => (this.get("read.explorechunks") ?? false) as boolean;
	setReadExplorechunks = (enabled: boolean) => this.set("read.explorechunks", enabled);

	getReadAnchorstyle = (): "full" | "kind" | "checksum" =>
		(this.get("read.anchorstyle") ?? "full") as "full" | "kind" | "checksum";
	setReadAnchorstyle = (style: "full" | "kind" | "checksum") => this.set("read.anchorstyle", style);

	// ─── Editing: bash interceptor ───────────────────────────
	getBashInterceptorEnabled = (): boolean => (this.get("bashInterceptor.enabled") ?? false) as boolean;
	setBashInterceptorEnabled = (enabled: boolean) => this.set("bashInterceptor.enabled", enabled);

	// ─── Editing: python execution ───────────────────────────
	getPythonToolMode = (): "both" | "notebook" | "script" =>
		(this.get("python.toolMode") ?? "both") as "both" | "notebook" | "script";
	setPythonToolMode = (mode: "both" | "notebook" | "script") => this.set("python.toolMode", mode);

	getPythonKernelMode = (): "session" | "ephemeral" =>
		(this.get("python.kernelMode") ?? "session") as "session" | "ephemeral";
	setPythonKernelMode = (mode: "session" | "ephemeral") => this.set("python.kernelMode", mode);

	getPythonSharedGateway = (): boolean => (this.get("python.sharedGateway") ?? true) as boolean;
	...
	getStartupCheckUpdate = (): boolean => (this.get("startup.checkUpdate") ?? true) as boolean;
	setStartupCheckUpdate = (enabled: boolean) => this.set("startup.checkUpdate", enabled);

	// ─── Interaction/P4: notifications ──────────────────────
	getCompletionNotify = (): boolean => (this.get("completion.notify") ?? true) as boolean;
	setCompletionNotify = (enabled: boolean) => this.set("completion.notify", enabled);

	getAskNotify = (): boolean => (this.get("ask.notify") ?? true) as boolean;
	setAskNotify = (enabled: boolean) => this.set("ask.notify", enabled);

	getAskTimeout = (): number => (this.get("ask.timeout") ?? 30) as number;
	setAskTimeout = (seconds: number) => this.set("ask.timeout", seconds);

	// ─── Interaction/P4: speech-to-text ─────────────────────
	getSttEnabled = (): boolean => (this.get("stt.enabled") ?? false) as boolean;
	setSttEnabled = (enabled: boolean) => this.set("stt.enabled", enabled);

	getSttLanguage = (): string => (this.get("stt.language") ?? "en") as string;
	setSttLanguage = (lang: string) => this.set("stt.language", lang);

	getSttModelName = (): string => (this.get("stt.modelName") ?? "base.en") as string;
	setSttModelName = (name: string) => this.set("stt.modelName", name);
