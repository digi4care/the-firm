...
import type {
	...
	AgentTool,
	ThinkingLevel,
} from "@digi4care/the-firm-agent-core";
import type { AssistantMessage, ImageContent, Message, Model, TextContent } from "@digi4care/the-firm-ai";
import { isContextOverflow, modelsAreEqual, resetApiProviders, supportsXhigh } from "@digi4care/the-firm-ai";
...
export class AgentSession {
	...
	private _buildRuntime(options: {
		...
		const editStreamingAbort = this.settingsManager.getEditStreamingAbort();
		const editBlockAutoGenerated = this.settingsManager.getEditBlockAutoGenerated();
		const baseToolDefinitions = this._baseToolsOverride
			...
			: createAllToolDefinitions(this._cwd, {
					...
					fuzzyThreshold: editFuzzyThreshold,
					streamingAbort: editStreamingAbort,
					blockAutoGenerated: editBlockAutoGenerated,
				},
				});
