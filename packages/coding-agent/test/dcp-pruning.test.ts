...
describe('Context Pruning (DCP)', () => {
	...
	describe('tool-pairing rule (CRITICAL)', () => {
		...
		it('preserves tool_result when its tool_use is kept', () => {
			...
			const result = applyContextPruning(messages, { ...DEFAULT_CONFIG, keepRecentCount: 4 });
			const tcIdx = result.findIndex((m) => m.role === 'assistant' && JSON.stringify(m.content).includes('tool_use'));
	...
	describe('deduplication rule', () => {
		it('removes duplicate assistant messages', () => {
			...
			const result = applyContextPruning(messages, { ...DEFAULT_CONFIG, keepRecentCount: 1, rules: ['deduplication', 'recency'] });
		...
		it('never prunes user messages', () => {
			...
			const result = applyContextPruning(messages, { ...DEFAULT_CONFIG, keepRecentCount: 10, rules: ['deduplication'] });
			expect(result).toHaveLength(3);
		});
	});
	...
	describe('error-purging rule', () => {
		it('removes errors that were followed by a successful retry', () => {
			...
			const result = applyContextPruning(messages, { ...DEFAULT_CONFIG, keepRecentCount: 3, rules: ['error-purging', 'recency', 'tool-pairing'] });
		...
		it('keeps errors that were not resolved', () => {
			...
			const result = applyContextPruning(messages, { ...DEFAULT_CONFIG, keepRecentCount: 10, rules: ['error-purging', 'recency', 'tool-pairing'] });
			const hasError = result.some((m) => JSON.stringify(m.content).includes('error: not found'));
	...
	describe('superseded-writes rule', () => {
		it('removes older writes to the same file when newer exists', () => {
			...
			const result = applyContextPruning(messages, { ...DEFAULT_CONFIG, keepRecentCount: 3, rules: ['superseded-writes', 'recency', 'tool-pairing'] });
