...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @digi4care/the-firm-ai <command> [provider]
...
Examples:
  ...
  npx @digi4care/the-firm-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @digi4care/the-firm-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @digi4care/the-firm-ai --help' for usage`);
	process.exit(1);
}
