...
```typescript
import { type AgentSessionEvent } from "@digi4care/the-firm";
...
import type { MomCustomTool, ToolAPI } from "@digi4care/the-firm-mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@digi4care/the-firm-ai` and `@digi4care/the-firm-mom` are aliased to the running mom bundle.
