...
```bash
npm install @digi4care/the-firm-mom
```
...
- run interactive coding agent session: `npx @digi4care/the-firm`
- enter `/login` command
...
Update mom anytime with `npm install -g @digi4care/the-firm-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
